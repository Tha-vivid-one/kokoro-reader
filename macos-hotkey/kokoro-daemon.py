...
LOG_FILE = os.path.expanduser("~/Models/kokoro/daemon.log")
IDLE_TIMEOUT = 1800  # 30 minutes with no requests → auto-shutdown (stays warm through a work session, frees ~300MB RAM when idle)
SAMPLE_RATE = 24000
...
def main():
    ...
    while True:
        ...
        # Handle one at a time (model isn't thread-safe for generation)
        handle_client(conn)
